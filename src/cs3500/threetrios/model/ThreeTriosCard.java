...
/**
 * Represents a card in the Three Trios game. This class encapsulates the properties and behaviors
 * associated with a game card, including its name and attack values in four cardinal directions.
 * Each card can perform directional attacks, and its strength is determined by these values.
 *
 * <p>The ThreeTriosCard class is central to game mechanics, as it defines how cards interact
 * based on their attributes, affecting gameplay outcomes through strategic placement and attack.
 */
public class ThreeTriosCard implements ICard {
  ...
  /**
   * Constructs a ThreeTriosCard with specified directional attack values and a name. Each attack
   * value represents the card's strength in a given direction, used in battle comparisons.
   *
  ...
  // Helper method: Validates the attack value of the card.
  ...
  /**
   * Checks if the card's name matches a given name.
   *
  ...
  /**
   * Provides a string representation of the card, formatted as "CardName X Y Z W" where each
   * letter represents an attack value (with 10 represented as 'A').
   *
  ...
  // Helper method: Converts attack values to string, handling 10 as 'A'.
  ...
  // Helper method: Constructs the string representation of attack values for the card.
   ...
   * @return true if the given object is a ThreeTriosCard with the same properties, false otherwise
   */
  @Override
  public boolean equals(Object obj) {
    ...
    return this.matchesName(card.cardName)
